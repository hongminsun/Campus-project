        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(supSaleAttr, index) in spuSaleAttrList"
                :key="supSaleAttr.id"
              >
                <!-- 属性名字   颜色 -->
                ...
                <dd
                  ...
                  ) in supSaleAttr.spuSaleAttrValueList"
...
export default {
  ...
  methods: {
    ...
    async addShopCar() {
      // 1.发请求----将产品加入到数据库中
      // 2.服务器存储成功-----进行路由跳转传递参数  (利用仓库中返回的是一个Promise 来判断成功还是失败)
      // 3.失败，向用户进行提示
      try {
        ...
        // 进行路由跳转 并将产品的信息带给下一个路由组件
        // 一些简单的数据 skuNum 通过query形式传递给路由组件
        // 复杂数据 skuInfo(里面是一层一层的对象)  通过会话存储(不持久，会话结束数据就消失)
        // 本地存储和会话存储 存储的都是字符串
